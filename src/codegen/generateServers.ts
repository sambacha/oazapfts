...
function createTemplate(url: string): TemplateLiteral {
  ...
  return factory.createTemplateExpression(factory.createTemplateHead(tokens[0]), [
    ...chunks.map(([expression, literal], i) => {
      return factory.createTemplateSpan(
        factory.createIdentifier(expression),
        (i === chunks.length - 1 ? factory.createTemplateTail : factory.createTemplateMiddle)(
          literal,
        ),
      );
    }),
...
export default function generateServers(servers: OpenAPIV3.ServerObject[]) {
  const props = servers.map((server, i) => {
    return [serverName(server, i), generateServerExpression(server)] as [string, ts.Expression];
  });
