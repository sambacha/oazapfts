...
export function runtime(defaults: RequestOpts) {
  ...
  async function fetchJson<T extends ApiResponse>(url: string, req: FetchRequestOpts = {}) {
  ...
  async function fetchBlob<T extends ApiResponse>(url: string, req: FetchRequestOpts = {}) {
    const res = await doFetch(url, req);
    let data;
    try {
      data = await res.blob();
